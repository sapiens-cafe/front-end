    ...
    >
      ...
      >
        <div className="flex justify-between items-center">
          ...
          {plan.featured && (
            <div
              className={cn('font-medium text-xs px-3 py-1 rounded-full relative bg-neutral-900')}
            >
        ...
        <div className="mt-8">
          ...
          <span className={cn('text-4xl font-bold', plan.featured && 'text-black')}>
        ...
        <Button
          ...
          className={cn(
            'w-full mt-10 mb-4',
            plan.featured && 'bg-black text-white hover:bg-black/80 hover:text-white'
          )}
          onClick={onClick}
        >
          {plan?.CTA?.text}
...
const Step = ({
  ...
  return (
    <div className="flex items-start justify-start gap-2 my-4">
      ...
      <div className={cn('font-medium text-white text-sm', featured && 'text-black')}>
        {children}
      </div>
    </div>
  );
...
const Divider = ({ featured }: { featured?: boolean }) => {
  return (
    <div className="relative">
      ...
      >
        <IconPlus
          className={cn('h-3 w-3 [stroke-width:4px] text-neutral-300', featured && 'text-black')}
        />
      </div>
    </div>
  );
